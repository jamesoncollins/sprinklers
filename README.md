...
2. **Try the built-in demo.** Click **Load TPC Sawgrass Demo** to immediately open an illustrative satellite-based golf-course project using Rain Bird EAGLE 900 rotors and preconfigured zones, grass areas, and sprinklers.
3. **Load sprinkler data.** The app auto-loads the built-in CSV catalog, and you can import additional manufacturer CSV rows when you need a different head or nozzle. Pick the manufacturer, head model, nozzle model, and rated catalog pressure for each sprinkler.
4. **Create zones.** Add one zone per valve/runtime group. For each zone, enter static pressure, optional dynamic pressure, open-flow supply, and water share factor. Static pressure is the no-flow gauge reading, open-flow supply is the estimated source flow at 0 PSI, and dynamic pressure is an optional measured gauge reading while that zone is actually running.
5. **Place and orient sprinklers.** Add sprinklers to the canvas, assign each one to a zone, set its arc or rectangular throw orientation, and use the coverage overlay to check head-to-head spacing and dry spots. Arc orientation is the left-hand lock angle; rectangular orientation points forward from the head.
6. **Read the analysis.** Zone cards and summary tables report solved operating pressure, total actual flow, per-head effective throw, and precipitation rates. Use these values to rebalance nozzles, split overloaded zones, or adjust runtimes.
7. **Save your work.** Export project JSON to preserve site settings, map view, imported catalog choices, zones, sprinklers, and analysis-related settings for later editing.
